...
def sense_atrium(fp):
    ...
    for i in range(int(PVARP_length), int(VAI_length)):
        if i%50 == 0:
            capacitors.sort()
        for j in range(4):
            charge_sc = min(rate_at_which_supercapacitor_is_charged, max_charge_in_supercapacitor - capacitors[j])
            battery_capacity -= charge_sc
            capacitors[j] += charge_sc
...
def pace_atrium():
    ...
    required_charge = current_to_pace_atrium * pulse_duration * coulomb_to_charge_units * 1e-6
    n = allocate_capacitors(required_charge)
    ...
    if(n != 0):
        for i in range(n):
            if capacitors[7-i] < required_charge:
               required_charge -= capacitors[7-i]
               capacitors[7-i] = 0
            else:
                capacitors[7-i] -= required_charge
                required_charge = 0
    else:
        pa_by_battery += 1
...
def sense_ventricle(fp):
    ...
    return signal
def allocate_capacitors(required_charge):
    capacitors.sort()
    temp = 0
    for i in range(8):
        temp += capacitors[7-i]
        if temp >= required_charge:
            return i+1
    return 0    

def pace_ventricle():
    global present_charge_in_supercapacitor, battery_capacity, pv_by_battery
    required_charge = current_to_pace_ventrium * pulse_duration * coulomb_to_charge_units * 1e-6
    n = allocate_capacitors(required_charge)
